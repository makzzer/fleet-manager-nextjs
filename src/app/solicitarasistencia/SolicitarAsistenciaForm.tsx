// /app/solicitarasistencia/SolicitarAsistenciaForm.tsx
...
const SolicitarAsistenciaForm: React.FC<SolicitarAsistenciaFormProps> = ({ vehicleIdFromQuery }) => {
  ...
  useEffect(() => {
    if (vehicleIdFromQuery && reservas.length > 0) {
      const userReservas = reservas.filter((reserva) => reserva.user_id === authenticatedUser?.id);
      const matchingReserva = userReservas.find((reserva) => reserva.vehicle_id === vehicleIdFromQuery);
      if (matchingReserva) {
        setSelectedReserva(matchingReserva.vehicle_id);
      } else {
        Swal.fire(
          'No tienes reservas',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirigir a la página "misreservas"
...
export default SolicitarAsistenciaForm;
